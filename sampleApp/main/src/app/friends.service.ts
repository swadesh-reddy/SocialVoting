    ...
    getAllFriends(status)
{
    ...
    this.token = this.auth.loadToken();
    const headers = this._headers.append('Authorization', 'Bearer ' + this.token);
    return this.http.post<Friend>("https://publicserver.localtunnel.me/users/checkFriends", username, { headers: headers });
...
addNotification(product) {
    ...
    cleanNotification(product) {
    this.token = this.auth.loadToken();
    const headers = this._headers.append('Authorization', 'Bearer ' + this.token);
    headers.set('Content-Type', 'multipart/form-data');
    return this.http.post<Notification>("https://publicserver.localtunnel.me/users/cleanNotification", product, { headers: headers });
